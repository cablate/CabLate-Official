...
從服務頁帶入預選路線時，服務選擇器必須出現在會隨選擇變動的 Hero 文案之前；對應的路線專屬題目要在主要表單程式完成初始化前先展開，避免初始隱藏造成閃爍或誤判。切換後，服務摘要與任何適用的費用條件要在控制項旁立即更新，不以自動捲動要求手機使用者回頭尋找變化。陪跑與諮詢的折抵條件沿用公開方案：先做 NT$5,000 專案諮詢者，14 天內加入陪跑可全額折抵，陪跑剩餘 NT$34,800；不得省略 14 天條件或改寫成所有人皆有的折扣。
